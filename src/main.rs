...
async fn doit() -> Result<(), Box<dyn std::error::Error>> {
    ...
    while !input.eq_ignore_ascii_case("quit") {
        ...
        if input.eq_ignore_ascii_case("test") {
            let client = b.clone();
            match  client.queue_message(openiap_client::QueueMessageRequest::byqueuename("mohammad1", "{\"name\":\"Allan\"}", true)).await {
                Ok(response) => {
                    println!("Queued message to {:?} with reply to {:?}", response.queuename, response.replyto);
                }
                Err(e) => {
                    println!("Failed to queue message: {:?}", e);
                }               
            };
        }
